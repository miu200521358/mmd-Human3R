...
def head_factory(
    ...
    elif head_type == "dpt" and output_mode == "pts3d+pose+smpl":
        return DPTPts3dPoseSMPL(net, has_conf, has_rgb, has_pose, has_msk)
    elif head_type == "dpt" and output_mode == "naive":
        return NaiveDPTPts3dPoseSMPL(net, has_conf, has_rgb, has_pose)
    else:
        raise NotImplementedError(f"unexpected {head_type=} and {output_mode=}")
